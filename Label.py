...
class Example(Frame):
    def __init__(self, parent):
        ...
        self.fileList = []
        self.fileIndex = -1
        self.filefolder = './data'
        self.parent = parent
        self.fileName = ""
        self.history = deque(maxlen=20)
        self.currentContent = deque(maxlen=1)
        self.pressCommand = {'t':"Task",
                             'm':"Method",
                             'd':"Dataset",
                             'r':"Result"}
    ...
    def initUI(self):
        ...
        abtn = Button(self, text="OpenFolder", command=self.onOpen)
        abtn.grid(row=1, column=self.textColumn +1)

        ubtn = Button(self, text="Next", command=self.NextFile)
        ubtn.grid(row=2, column=self.textColumn +1, pady=4)

        exportbtn = Button(self, text="Last", command=self.LastFile)
    ...
    def changeFile(self, num):
        if num < 0 or num > len(self.fileList):
            print "change File Error"
            return 
        fl = self.fileList[num]
        self.text.delete("1.0",END)
        text = self.readFile(os.path.join(self.filefolder, fl))
    ...
    def onOpen(self):
        ...
        if fl != '':
            ...
            self.fileIndex = 0
            if not os.path.exists(os.path.join(fl, 'labeled')):
                os.mkdir(os.path.join(fl, 'labeled'))
            if len(self.fileList) == 0:
                tkMessageBox.showinfo(title='Error', message='No txt file in dir')
            else:
                self.changeFile(self.fileIndex)
    def NextFile(self):
        self.fileIndex += 1
    ...
    def replaceString(self, content, string, replaceType, cursor_index):
        if replaceType in self.pressCommand:
            new_string = "<"+replaceType +">"+ string + "</" +replaceType+">" 
    ...
    def writeFile(self, fileName, content, newcursor_index):
        if len(fileName) > 0:
            if ".ann" in fileName:
                ...
                new_name = os.path.join(self.filefolder, 'labeled', real_file_name)
                ann_file = open(new_name, 'w')
                ann_file.write(content)
                ann_file.close()   
